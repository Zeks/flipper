...
alter table FanficData.fandomindex add column updated timestamp;
...
CREATE INDEX if not exists I_FANDOMINDEX_UPDATED ON FanficData.fandomindex (updated ASC);
...
CREATE INDEX if not exists I_FURL_ID ON FanficData.fandomurls (global_id ASC);
...
alter table FanficData.recommenders add column earliest_favourited_fic_date timestamp;
 ...
 create table if not exists FanficData.AuthorFavouritesStatistics (author_id INTEGER PRIMARY KEY NOT NULL UNIQUE);
 alter table FanficData.AuthorFavouritesStatistics add column favourites integer default 0;                  -- how much stuff did the author favourite;
 alter table FanficData.AuthorFavouritesStatistics add column favourites_wordcount integer default 0;        -- how much stuff did the author favourite;
 alter table FanficData.AuthorFavouritesStatistics add column average_words_per_chapter real;
 
 alter table FanficData.AuthorFavouritesStatistics add column esrb_type integer default -1;                          -- agnostic/kiddy/mature;
 alter table FanficData.AuthorFavouritesStatistics add column prevalent_mood integer default -1;             -- sad/neutral/positive as categorized by genres;
 
 alter table FanficData.AuthorFavouritesStatistics add column most_favourited_size integer default -1;       -- small/medium/large/huge;
 alter table FanficData.AuthorFavouritesStatistics add column favourites_type integer default -1;            -- tiny(<50)/medium(50-500)/large(500-2000)/bullshit(2k+);
 
 alter table FanficData.AuthorFavouritesStatistics add column average_favourited_length real default null;  -- excluding the biggest outliers if there are not enough of them;
 alter table FanficData.AuthorFavouritesStatistics add column favourite_fandoms_diversity real;              -- how uniform are his favourites relative to most popular fandom;
 alter table FanficData.AuthorFavouritesStatistics add column explorer_factor real;                          -- deals with how likely is the author to explore otherwise unpopular fics;
 alter table FanficData.AuthorFavouritesStatistics add column mega_explorer_factor real;                     -- deals with how likely is the author to explore otherwise unpopular fics;
 
 alter table FanficData.AuthorFavouritesStatistics add column crossover_factor real;                         -- how willing is he to read crossovers;
 alter table FanficData.AuthorFavouritesStatistics add column unfinished_factor real;                        -- how willing is he to read stuff that isn't finished;
 alter table FanficData.AuthorFavouritesStatistics add column esrb_uniformity_factor real;                   -- how faithfully the author sticks to the same ESRB when favouriting. Only  M/everything else taken into account;
 alter table FanficData.AuthorFavouritesStatistics add column esrb_kiddy real;
 alter table FanficData.AuthorFavouritesStatistics add column esrb_mature real;
 
 alter table FanficData.AuthorFavouritesStatistics add column genre_diversity_factor real;                  -- how likely is the author to stick to a single genre;
 alter table FanficData.AuthorFavouritesStatistics add column mood_uniformity_factor real;
 alter table FanficData.AuthorFavouritesStatistics add column mood_sad real;
 alter table FanficData.AuthorFavouritesStatistics add column mood_neutral real;
 alter table FanficData.AuthorFavouritesStatistics add column mood_happy real;
 
 alter table FanficData.AuthorFavouritesStatistics add column crack_factor real;
 alter table FanficData.AuthorFavouritesStatistics add column slash_factor real;
 alter table FanficData.AuthorFavouritesStatistics add column not_slash_factor real;
 alter table FanficData.AuthorFavouritesStatistics add column smut_factor real;
 
 alter table FanficData.AuthorFavouritesStatistics add column prevalent_genre varchar default null;

 alter table FanficData.AuthorFavouritesStatistics add column size_tiny real;
 alter table FanficData.AuthorFavouritesStatistics add column size_medium real;
 alter table FanficData.AuthorFavouritesStatistics add column size_large real;
 alter table FanficData.AuthorFavouritesStatistics add column size_huge real;
 
 alter table FanficData.AuthorFavouritesStatistics add column first_published TIMESTAMP;
 alter table FanficData.AuthorFavouritesStatistics add column last_published TIMESTAMP;
...
-- which type of fandoms they are in : anime, games, books....  ;
 ...
 create table if not exists FanficData.AuthorFavouritesGenreStatistics (author_id INTEGER PRIMARY KEY NOT NULL UNIQUE);
 alter table FanficData.AuthorFavouritesGenreStatistics add column General_ real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Humor real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Poetry real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Adventure real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Mystery real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Horror real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Drama real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Parody real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Angst real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Supernatural real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Suspense real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Romance real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column NoGenre real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column SciFi real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Fantasy real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Spiritual real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Tragedy real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Western real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Crime real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Family real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column HurtComfort real default 0;
 alter table FanficData.AuthorFavouritesGenreStatistics add column Friendship real default 0;
 ...
 create table if not exists FanficData.FicGenreStatistics (fic_id INTEGER PRIMARY KEY NOT NULL UNIQUE);
 alter table FanficData.FicGenreStatistics add column General_ real default 0;
 alter table FanficData.FicGenreStatistics add column Humor real default 0;
 alter table FanficData.FicGenreStatistics add column Poetry real default 0;
 alter table FanficData.FicGenreStatistics add column Adventure real default 0;
 alter table FanficData.FicGenreStatistics add column Mystery real default 0;
 alter table FanficData.FicGenreStatistics add column Horror real default 0;
 alter table FanficData.FicGenreStatistics add column Drama real default 0;
 alter table FanficData.FicGenreStatistics add column Parody real default 0;
 alter table FanficData.FicGenreStatistics add column Angst real default 0;
 alter table FanficData.FicGenreStatistics add column Supernatural real default 0;
 alter table FanficData.FicGenreStatistics add column Suspense real default 0;
 alter table FanficData.FicGenreStatistics add column Romance real default 0;
 alter table FanficData.FicGenreStatistics add column NoGenre real default 0;
 alter table FanficData.FicGenreStatistics add column SciFi real default 0;
 alter table FanficData.FicGenreStatistics add column Fantasy real default 0;
 alter table FanficData.FicGenreStatistics add column Spiritual real default 0;
 alter table FanficData.FicGenreStatistics add column Tragedy real default 0;
 alter table FanficData.FicGenreStatistics add column Western real default 0;
 alter table FanficData.FicGenreStatistics add column Crime real default 0;
 alter table FanficData.FicGenreStatistics add column Family real default 0;
 alter table FanficData.FicGenreStatistics add column HurtComfort real default 0;
 alter table FanficData.FicGenreStatistics add column Friendship real default 0;
 
 CREATE INDEX if not exists I_FGS_FID ON FanficData.FicGenreStatistics (fic_id ASC);
  
  -- need cluster relations in separate table;
 create table if not exists FanficData.AuthorFavouritesFandomRatioStatistics (author_id INTEGER);
 
 alter table FanficData.AuthorFavouritesFandomRatioStatistics add column fandom_id integer default null;
 alter table FanficData.AuthorFavouritesFandomRatioStatistics add column fandom_ratio real default null;
 alter table FanficData.AuthorFavouritesFandomRatioStatistics add column fic_count integer default 0;
 
 alter table FanficData.AuthorFavouritesFandomRatioStatistics add primary key (author_id, fandom_id);
 
 CREATE INDEX if not exists I_AFRS_AID_FAID ON FanficData.AuthorFavouritesFandomRatioStatistics (author_id ASC, fandom_id asc);
...
create or replace view FanficData.AuthorMoodStatistics AS
select 
...
from FanficData.AuthorFavouritesGenreStatistics;

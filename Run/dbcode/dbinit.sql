...
CREATE TABLE if not exists RecommendationTagStats (author_id INTEGER NOT NULL , fic_count INTEGER, match_count integer, match_ratio double, tag varchar,  PRIMARY KEY (tag, author_id));
CREATE INDEX if not exists  I_REC_STATS_TAG ON RecommendationTagStats (tag ASC);
...
CREATE TABLE if not exists RecommendationFicStats (fic_id INTEGER NOT NULL , sumrecs INTEGER default 0, tag varchar,  PRIMARY KEY (fic_id, tag));
CREATE INDEX if not exists  I_FIC_STATS_PK ON RecommendationFicStats (fic_id asc, tag ASC);
CREATE INDEX if not exists  I_FIC_STATS_FIC ON RecommendationFicStats (fic_id asc);
CREATE INDEX if not exists  I_FIC_STATS_TAG ON RecommendationFicStats (tag asc);

...
Rectangle{
    ...
    Image {
        ...
        opacity:{
            if(!custom_opacity)
                return ficIsSnoozed ? opacity_full : opacity_gray
            else
                return custom_opacity_value
        }
        source: {
            var selection = ficIsSnoozed ? icon_colored : icon_gray
            return selection;
        }
        ...
        MouseArea{
            ...
            onClicked : {
                ...
                if(!clickable)
                    return;
                ficIsSnoozed ?  lvFics.removeSnooze(indexOfThisDelegate) : lvFics.addSnooze(indexOfThisDelegate);
            }
        }
    }
}
